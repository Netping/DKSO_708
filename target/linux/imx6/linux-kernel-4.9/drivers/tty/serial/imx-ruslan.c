...
struct imx_port {
	...
	unsigned int		have_rtsgpio:1; //ruslan
...
//			imx_port_rts_inactive(sport, &temp);
			imx_port_rts_active(sport, &temp);// ruslan
		else
//			imx_port_rts_active(sport, &temp);
			imx_port_rts_inactive(sport, &temp); // ruslan
		temp |= UCR2_RXEN;
...
//					imx_port_rts_inactive(sport, &ucr2);
					imx_port_rts_active(sport, &ucr2);//ruslan
				else
//					imx_port_rts_active(sport, &ucr2);
					imx_port_rts_inactive(sport, &ucr2);//ruslan
			} else {
			    ...
			    struct serial_rs485 *rs485conf)
{
	...
	if (!sport->have_rtscts && !sport->have_rtsgpio)    // ruslan
		rs485conf->flags &= ~SER_RS485_ENABLED;     // ruslan
//	if (!sport->have_rtscts)
//		rs485conf->flags &= ~SER_RS485_ENABLED;

	if (rs485conf->flags & SER_RS485_ENABLED) {
		/* Enable receiver if low-active RTS signal is requested */// ruslan
		if (sport->have_rtscts &&  !sport->have_rtsgpio &&	// ruslan
			!(rs485conf->flags & SER_RS485_RTS_ON_SEND))    // ruslan
			    rs485conf->flags |= SER_RS485_RX_DURING_TX; // ruslan
		...
		struct platform_device *pdev)
{
	...
	if (of_property_read_bool(np, "linux,rs485-enabled-at-boot-time"))// ruslan
		sport->port.rs485.flags |= SER_RS485_ENABLED; // ruslan

	if (of_get_property(np, "rts-gpios", NULL))   // ruslan

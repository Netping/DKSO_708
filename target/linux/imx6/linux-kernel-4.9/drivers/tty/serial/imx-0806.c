...
static int serial_imx_probe(struct platform_device *pdev)
{
	...
	sport->port.rs485.flags |= 					
		SER_RS485_RTS_ON_SEND;		// ruslan | SER_RS485_RX_DURING_TX;

if ( sport->have_rtscts == 1 )   // ruslan
        {
           printk("RTS enable");	// ruslan
...
//	struct imx_port *sport = (struct imx_port *)port;
	...
	temp = readl_relaxed(sport->port.membase + UCR2);  				//
		if (SER_RS485_ENABLED) {						//SER_RS485_ENABLE
		    imx_port_rts_inactive(sport, &temp);			//
		    printk("RTS disable");				// ruslan.
		}
